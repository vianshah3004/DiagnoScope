...
const AdvancedAnalysis = () => {
    ...
    const handleSmartAnalysis = async (manualFile = null, manualFileName = null) => {
        ...
        try {
            ...
            if (diseaseType === 'ECG') analysisType = 'ecg';
            if (diseaseType === 'Pneumonia') analysisType = 'lungs';
            if (diseaseType === 'Skin Lesion') analysisType = 'skin';
            ...
            if (diseaseType === 'ECG') port = 8003;
            if (diseaseType === 'Pneumonia') port = 8004;
            if (diseaseType === 'Skin Lesion') port = 8005;
            ...
            if (resultData.results && resultData.results.length > 0) {
                ...
                // Capture ECG Details if present
                if (res.ecg_details) {
                    setEcgDetails(res.ecg_details);
                }
            }
        } catch (error) {
            console.error(error);
    ...
    const handleDownloadReport = async () => {
        setIsLoading(true);
        try {
            // 1. Determine Port & Modality
            let port = 8000;
            let modality = "X-Ray";
            if (diseaseType === 'Tumor') { port = 8001; modality = "MRI - Brain"; }
            if (diseaseType === 'Diabetic Retinopathy Scan') { port = 8002; modality = "Retinal Scan"; }
            if (diseaseType === 'ECG') { port = 8003; modality = "ECG"; }
            if (diseaseType === 'Pneumonia') { port = 8004; modality = "Lung Scan"; }
            if (diseaseType === 'Skin Lesion') { port = 8005; modality = "Skin Analysis"; }
            ...
            // Fallback for original if missing
            ...
            const reportReq = {
                ...
                diagnosis: diagnosis,
                confidence: confidence / 100, // API expects 0-1
                metrics: metrics,
                images: images,
                modality: modality
            };
            ...
            // 5. Call API
            const response = await fetch(`http://127.0.0.1:${port}/generate_report`, {
                method: 'POST',
    ...
    useEffect(() => {
        if (location.state) {
            ...
            if (location.state.fileData) {
                ...
                // Auto-run analysis if it's a Tumor, DR, ECG, Pneumonia, or Skin case
                const autoRunTypes = ['Tumor', 'Diabetic Retinopathy Scan', 'ECG', 'Pneumonia', 'Skin Lesion'];
            ...
            <div className="advanced-page page-transition container">
                ...
                <div className="control-bar glass-panel">
                    ...
                    <div className="control-group right">
                        ...
                        <button className="btn-outline small" onClick={() => handleSmartAnalysis()} disabled={isLoading} style={{ marginLeft: '10px', borderColor: clinical.color, color: clinical.color }}>
                            <Sparkles size={16} style={{ marginRight: '5px' }} /> Re-Run Analysis
                        </button>
                    </div>
                </div>
        ...
        <div className="advanced-page page-transition container">
            ...
            <div className="control-bar glass-panel">
                ...
                <div className="control-group right">
                    ...
                    {(diseaseType !== 'Tumor' && diseaseType !== 'Diabetic Retinopathy Scan' && diseaseType !== 'ECG' && diseaseType !== 'Pneumonia' && diseaseType !== 'Skin Lesion') && (
            ...
            {diseaseType === 'ECG' && !ecgDetails && !isLoading && (
                <div className="glass-panel" style={{ padding: '60px', textAlign: 'center' }}>
                    ...
                    <p style={{ color: '#888' }}>Click "Smart Auto Detection" to analyze the heart rhythm data.</p>
                </div>
            )}
